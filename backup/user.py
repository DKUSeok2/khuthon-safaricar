...
def load_image(image_filename, size=(320, 180)):
    image_dir = os.path.join(os.path.dirname(__file__), "images")  # images/ 폴더 경로
    image_path = os.path.join(image_dir, image_filename)

    try:
        img = Image.open(image_path)
    except FileNotFoundError:
        img = Image.new("RGB", size, color="#cccccc")  # 대체 이미지
    img = img.resize(size)
...
class FarmApp(tk.Tk):
    ...
    def create_farm_card(self, parent, farm, farm_id):
        ...
        # image_path 키가 없을 경우 default.jpg 사용
        image_filename = farm.get("image_path", "default.jpg")
        photo = load_image(image_filename)
